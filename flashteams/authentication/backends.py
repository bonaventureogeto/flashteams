...
class JWTAuthentication(authentication.BaseAuthentication):
    ...
    #bearer
    authe_header_prefix = 'Bearer'.lower()
    ...
    def authenticate(self, request):
        ...
        if not auth_header:
            # if we get no authorization header, we do not attempt to authenticate
            return None

        if len(auth_header) == 1:
            # we expect the length to be 2, so this is an invalid header
            # Do not attempt to authenticate
            return None

        elif len(auth_header) > 2:
            # invalid token heaer. The length must be 2
            # do not attempt to authenticate
            return None

        # We have to decode both the prefix and token because they are in bytes,
        # and the JWT library we use can't handle bytes.
        prefix = auth_header[0].decode('utf-8')
        token = auth_header[1].decode('utf-8')

        if prefix.lower() != self.authe_header_prefix:
            # the auth header prefix should only be 'Bearer'.
            # if otherwise, don't attempt to authenticate
            return None
    ...
    def _authenticate_credentials(self, request, token):
        ...
        """
        try:
            payload = jwt.encode(token, settings.SECRET_KEY)
        ...
        if not user.is_active:
            msg = 'Forbiden! This user has been deactivated.'
            raise exceptions.AuthenticationFailed(msg)
